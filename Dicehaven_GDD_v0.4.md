...
*Revision 2026-07-12 (3): Combat Speed Bars animerer fill på compositor-laget uafhængigt af React-renders. Clock-reglerne kører fortsat frame-præcist, mens countdown-state throttles til 100 ms for at undgå mobil-jank (§12).*
...
* **Combat Speed Bars:** Player Roll Speed og Enemy Attack Speed er custom `scaleX`-fills frem for native progress. Fill bevæger sig som en lineær, GPU/compositor-animeret cyklus og er dermed ikke bundet til Reacts renderfrekvens; countdown og ARIA-state opdateres i læsbare 100 ms-trin. Begge viser `X.Xs until roll/attack`, det fulde interval og et tydeligt reset. Player bruger grøn fill; enemy bruger varm rød/orange fill, så de to samtidige rytmer kan aflæses uden kun at stole på tekst eller farve.
